...
const { parseOrderItemsFromBody } = require('../src/db');
...
test('parseOrderItemsFromBody lee arrays description/uom/quantity', () => {
  ...
  assert.deepEqual(items, [
    { description: 'Varilla 3/8', uom: 'PZ', quantity: 12 },
    { description: 'Cemento', uom: 'KG', quantity: 50 },
  ]);
});
...
test('parseOrderItemsFromBody acepta valores sueltos (no array)', () => {
  ...
  assert.deepEqual(items, [{ description: 'Arena', uom: 'M3', quantity: 2.5 }]);
});
